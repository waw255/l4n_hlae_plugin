...
#ifndef SOURCESDK_CS2_TIER1_STRTOOLS_H
#define SOURCESDK_CS2_TIER1_STRTOOLS_H

#include "../../../../AfxHookSource/SourceSdkShared.h"
#include "../tier0/platform.h" //#include "../tier0/basetypes.h"
...
SOURCESDK_abstract_class IParsingErrorListener
{
	virtual void OnParsingError(CBufferString &error_msg) = 0;
};

#define SOURCESDK_CS2_PARSING_FLAG_NONE							(0)
#define SOURCESDK_CS2_PARSING_FLAG_ERROR_ASSERT					(1 << 0) // Triggers debug assertion on parsing errors, the default state
#define SOURCESDK_CS2_PARSING_FLAG_SKIP_ASSERT					(1 << 1) // Internal flag that is set when assertion is triggered, could also be used to prevent debug assertions
#define SOURCESDK_CS2_PARSING_FLAG_EMIT_WARNING					(1 << 2) // Emits global console warning on parsing errors, the default state
#define SOURCESDK_CS2_PARSING_FLAG_SKIP_WARNING					(1 << 3) // Internal flag that is set when global warning is emitted, could also be used to prevent warning messages
#define SOURCESDK_CS2_PARSING_FLAG_SILENT							(1 << 4) // Won't call callback when parsing errors are encountered
#define SOURCESDK_CS2_PARSING_FLAG_ERROR_IF_EMPTY					(1 << 5) // Emits parsing error if the input string was empty or NULL
#define SOURCESDK_CS2_PARSING_FLAG_UNK006							(1 << 6)
#define SOURCESDK_CS2_PARSING_FLAG_USE_BASE_AUTO					(1 << 7) // Use auto detection of a number base when parsing (uses https://en.cppreference.com/w/cpp/string/basic_string/stol under the hood, so same base rules applies)
#define SOURCESDK_CS2_PARSING_FLAG_USE_BASE_16					(1 << 8) // Use base of 16 when parsing
...
int V_snprintf( char *pDest, int destLen, const char *pFormat, ... ) SOURCESDK_CS2_FMTFUNCTION( 3, 4 );
...
#endif	// SOURCESDK_CS2_TIER1_STRTOOLS_H
